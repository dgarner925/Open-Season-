...
- **D4 Home stat tiles.** DECIDED (David, 2026-08-27, from A/B/C render): KEEP the three stat boxes (option A) — retokenized to lang palette (surface #141110, hair border, radius.card, Serif numbers, sans labels). Home gets a sanctioned-exception note: stat tiles + species badge tiles are allowed boxes; the language's hairline purism applies to lists and detail screens. This also signals David's taste for compact controls — render options before applying sentence-style treatments elsewhere (D10).
...
**Exempt:** admin.tsx (internal; stays legacy), LaunchQuote (exception per D11), species/[id].tsx, calendar/Seasons, applications/Tags, regs (list), settings/Profile (ALL REVERTED to their tile/card designs at David's request 2026-08-28 — he wants the tiles on the tab screens and the original species page; do not reconvert). The v2 language's home turf is now: detail screens (season/window/party), search, paywall, sign-in, forms, onboarding, alerts, small screens, Home hybrid.
