-- notifications_due(): add the Weekend Brief branch — Fridays only, one row
-- per Pro user with brief-worthy content (opener Fri-Sun, closer Fri-Sun, or
-- deadline within 7 days), composed of up to 3 lines. Deduped per week via a
-- deterministic md5 uuid of (user, date). Empty weekends send nothing.
    ...
    from (
      ...
      select * from (
        select
          ...
          2 as priority,
          s.close_date as d,
          format('%s closes %s in %s — last days', sp.name,
      ...
      select * from (
        select
          ...
          3 as priority,
          w.closes_at as d,
          format('%s %s draw closes %s', st.code, sp.name, to_char(w.closes_at, 'Dy Mon DD')) as line,
