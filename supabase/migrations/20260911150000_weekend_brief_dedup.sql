-- Weekend Brief dedup (2026-09-11): one sentence per fact. Two seasons of the
-- same species opening the same day in the same state (GA Deer Archery +
-- Extended Archery, both Sep 12) produced "Deer opens Saturday in Georgia"
-- twice in one brief (David's screenshot). The followed arms now dedupe by
-- composed line text before the 3-line cap, like the discovery arm always
-- did. Identical to 20260905120000 otherwise; original rationale below.
--
-- Weekend Brief grammar (2026-09-05): plural species names take plural verbs
-- ("Geese open Saturday", not "Geese opens"). Original discovery rationale
-- below.
--
-- Weekend Brief discovery (2026-09-04, David): the brief speaks about the
-- user's STATES, not only their follows. Follows stay first-class (priorities
-- 1-3 lead and the 3-line cap protects them); a new priority-4 arm fills any
-- remaining room with openers this weekend in the user's tracked states
-- (follow states plus resident state) for species they DON'T follow - so a
-- Georgia hunter hears "Dove opens Saturday in Georgia" without following
-- dove, and a user with zero follows still gets their resident state's
-- weekend. Muting a followed method keeps it out of both arms.
...
as $$
  ...
  from (
    ...
    from (
      ...
      from (
      select distinct u.user_id, u.priority, u.d, u.line
      from (
      select * from (
        select
          f.user_id,
          1 as priority,
          s.open_date as d,
          format('%s %s %s in %s', sp.name,
                 case when sp.name in ('Geese', 'Ducks') then 'open' else 'opens' end,
      ...
      select * from (
        select
          ...
          2 as priority,
          s.close_date as d,
          format('%s %s %s in %s — the last days', sp.name,
                 case when sp.name in ('Geese', 'Ducks') then 'close' else 'closes' end,
      ...
      union all
      -- Priority 4: discovery - weekend openers in the user's states for
      -- species they don't follow. Lowest priority, so it only fills room the
      -- user's own hunts left in the 3-line cap.
      select * from (
        select
          x.user_id,
          4 as priority,
          x.d,
          x.line,
          row_number() over (partition by x.user_id order by x.d, x.line) as rn
        from (
          select distinct
            ts.user_id,
            s.open_date as d,
            format('%s %s %s in %s', sp.name,
                   ...
                   st.name) as line
          from (
            select f.user_id, f.state_id from public.follows f
            union
            select p.id, p.resident_state_id from public.profiles p where p.resident_state_id is not null
          ) ts
          join public.seasons s on s.state_id = ts.state_id
          join public.states st on st.id = s.state_id
          join public.species sp on sp.id = s.species_id
          where s.status = 'published'
            and s.open_date between current_date and current_date + 2
            and not exists (
              ...
              where f2.user_id = ts.user_id and f2.state_id = s.state_id and f2.species_id = s.species_id
            )
        ) x
      ) disc where disc.rn <= 3
      ) u
      ) lines
    ) capped
  ...
  join public.profiles pr on pr.id = b.user_id and pr.is_premium and pr.weekend_brief
