  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AlertPref[]> => {
      const { data, error } = await supabase
        .from('follows')
        .select('id, state:states(code,name), species:species(key,name), alert_preferences(opener_offsets, deadline_offsets)')
        .eq('user_id', user!.id);
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await supabase
        ...
        .upsert(
          { follow_id: followId, opener_offsets: DEFAULT_OFFSETS, deadline_offsets: DEFAULT_OFFSETS },
